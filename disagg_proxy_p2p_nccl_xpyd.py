...
async def handle_request_async():
    try:
        ...
        if original_request_data is None:
            return {"error": "Invalid JSON"}, 400

        prefill_request = original_request_data.copy()
        # Set max_tokens=1 to trigger only prefill
        ...
        # ----------------------------
        # 🔥 Launch prefill in background (fire-and-forget)
        # ----------------------------
        async def run_prefill():
            try:
                ...
                print(f"[Prefill done] request_id={request_id}, time={t2 - t1:.3f}s")
            except Exception as e:
                print(f"[Prefill failed] request_id={request_id}, error: {e}")

        # Start prefill as a background task

        t1 = time.perf_counter()
        asyncio.create_task(run_prefill())
        t2 = time.perf_counter()
        print(f"waiting prefill time {t2 - t1} s")
    

        # ----------------------------
        # 🚀 Immediately start decode and stream response
        # ----------------------------
        decode_generator = forward_request(
            f"http://{decode_addr}/v1/completions",
            original_request_data,
            request_id
        )

        response = await make_response(decode_generator)
