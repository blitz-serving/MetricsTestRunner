...
#POLICIES=("round-robin-q" "least-bs-random" "llumnix-linear-04" "join-shortest-q-weight" "join-shortest-q-tuple" "bailian-impl-06" "dynamo-deterministic" "least-wait-token-q" "least-wait-token-random" "least-wait-token-bs" "least-wait-token-gated-bs" "join-shortest-q-ttft" "slo-serve-impl-q" "llmd-impl-q" "poly-serve-impl-q")
...
ROUTER_CFG="$SIM_CONFIG_DIR/vllm_router_new_fullargs.toml"
...
for bs in ${BATCH_SIZES[@]}; do
    for run_id in 1 2; do  # Run 3 times: r1, r2, r3
        TAG="batch${bs}_u0.9_flashinfer_predictions_r$run_id"
