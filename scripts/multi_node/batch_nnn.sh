...
POLICIES=("round-robin-q" "random-q" "join-shortest-q-weight" "join-shortest-q-tuple")
        ...
        while IFS= read -r -d '' dir; do
            ...
            if [ -f "$dir/client.jsonl" ]; then
                ...
                if [[ $basename_dir =~ ^[0-9]+_([a-zA-Z0-9_-]+)$ ]]; then
                    ...
                    case "$policy" in
                        round-robin-q|least-wait-token-q|bailian-impl-q|join-shortest-q-weight|join-shortest-q-tuple|random-q|bailian-impl-kv|bailian-impl-rqs|bailian-impl-tks)
